    ...
    <div className="p-6 space-y-6">
        ...
        {isDirector && (
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => toast('审批完成：相关数据已写入共享账务数据')}
            >
              审核当前凭证
            </Button>
            <Button
              ...
              variant="outline"
              disabled={!selectedVoucher}
              onClick={() => toast('已执行反审核：该凭证退回至待审核状态')}
            >
              反审核当前凭证
            </Button>
            <Button
              size="sm"
              variant="outline"
              onClick={() => toast('批量审核完成：共处理 3 张待审核凭证')}
            >
              批量审核
            </Button>
          </div>
      ...
      <Card className="elevation-1">
        <CardContent className="pt-4 flex flex-wrap items-center gap-3">
          ...
          <Input placeholder="凭证字号、摘要、制单人" className="h-8 text-xs w-48" />
      ...
      {/* ========== Tabs ========== */}
      <Tabs defaultValue="全部凭证" value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          <TabsTrigger value="待审核" className="text-xs">
            待审核<span className="ml-1 text-[11px] text-muted-foreground">3</span>
          </TabsTrigger>
          <TabsTrigger value="已审核" className="text-xs">
            已审核<span className="ml-1 text-[11px] text-muted-foreground">2</span>
          </TabsTrigger>
          <TabsTrigger value="已记账" className="text-xs">
            已记账<span className="ml-1 text-[11px] text-muted-foreground">3</span>
          </TabsTrigger>
          <TabsTrigger value="全部凭证" className="text-xs">
            全部凭证<span className="ml-1 text-[11px] text-muted-foreground">8</span>
          </TabsTrigger>
        </TabsList>
      </Tabs>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 text-sm flex-wrap">
                  ...
                  <div className="flex items-center gap-1.5 rounded-full bg-warning/10 px-3 py-1">
                    ...
                    <span className="text-xs text-muted-foreground">等待林主管审核</span>
                  </div>
                  <ArrowIcon />
                  <div className="flex items-center gap-1.5 rounded-full bg-muted px-3 py-1">
                    <Clock className="h-4 w-4 text-muted-foreground" />
                    <span className="text-muted-foreground font-medium">记账</span>
                    <span className="text-xs text-muted-foreground">尚未记账</span>
                  </div>
                </div>
